...
def login():
	...
	if password == '' or password == None:
		alert('You must set a password to continue..', 'And remember it all the time')
		while True:
			pw1 = search('Please enter your psswd 1st time: ')
			pw2 = search('Please re-enter your psswd SAME as 1st time')
			
			if pw1 in ['', False] or pw2 in ['', False]:	alert('Error', 'Password can not be blank')
			elif pw1 == pw2: break
			else: alert('Password does not match', 'Try again')
			
		alert('Your password entered is: "' + pw1 + '"', 'Pls keep it safe')
